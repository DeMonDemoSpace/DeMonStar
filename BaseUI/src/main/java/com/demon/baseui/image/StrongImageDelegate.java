...
public class StrongImageDelegate {
    private static final String TAG = "StrongImageDelegate";

    private Callback mCallback;

    private DisplayMetrics mDisplayMetrics;

    private StrongImageHandler mHandler;
    ...
    private StrongImage.ScaleType mScaleType = StrongImage.ScaleType.FIT_CENTER;
    ...
    public StrongImageDelegate(DisplayMetrics metrics, Callback callback) {
        mDisplayMetrics = metrics;
        mCallback = Utils.requireNonNull(callback);
        HandlerThread handlerThread = new HandlerThread(TAG);
        handlerThread.start();
        mHandler = new StrongImageHandler(handlerThread.getLooper());
        mZoomAnimator = ValueAnimator.ofFloat(0, 1f);
        mZoomAnimator.setDuration(StrongImage.DURATION_ZOOM);
    ...
    public void load(Bitmap bitmap) {
        load(new ImageBitmapDecoder(bitmap));
    }
    ...
    private void prepareDraw(Rect rect) {
        ...
        if (mImage.mImageSampleSize > sampleSize) {
            ...
            StrongImageCache.ImageCache imageCache = mImage.mImageCaches.get(sampleSize);
            if (imageCache != null) {
    ...
    public void setScaleType(StrongImage.ScaleType scaleType) {
    ...
    public StrongImage.ScaleType getScaleType() {
        return mScaleType;
    }
    ...
    private class Image {
        ...
        private Image(ImageDecoder decoder) {
            try {
                ...
                Log.e(TAG, "Image: 无法的访问图片");
                e.printStackTrace();
            }
            mImageCaches = new StrongImageCache(5,
    ...
    public static class ImageBitmapDecoder implements ImageDecoder {
        private Bitmap bitmap;

        public ImageBitmapDecoder(Bitmap bm) {
            bitmap = bm;
        }
        ...
        public BitmapRegionDecoder newRegionDecoder() throws IOException {
            ...
            return BitmapRegionDecoder.newInstance(isBm, false);
        }
    }
    ...
    private class StrongImageHandler extends Handler {

        public StrongImageHandler(Looper looper) {
            super(looper);
        }
