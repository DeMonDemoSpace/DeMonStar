...
public class SatelliteMenu extends ViewGroup implements OnClickListener {
    ...
    public SatelliteMenu(Context context, AttributeSet attrs, int defStyle) {
        ...
        // 获取自定义属性的值
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SatelliteMenu, defStyle, 0);

        int pos = a.getInt(R.styleable.SatelliteMenu_sm_position, POS_RIGHT_BOTTOM);
        switch (pos) {
            case POS_LEFT_TOP:
        ...
        mRadius = (int) a.getDimension(R.styleable.SatelliteMenu_sm_radius, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 150, getResources().getDisplayMetrics()));
        a.recycle();
